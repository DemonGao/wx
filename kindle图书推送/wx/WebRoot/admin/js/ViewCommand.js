...
var ViewCommand = (function(){
	var templet={
		userlist:[
			'<tr>',
				...
				'<td><button type="button" class="btn btn-warning" data-toggle="modal" data-target="#seeuserlist">查看详情</button></td>',
			'</tr>'
		].join(''),
		booklist:[
			'<tr>',
	...
	function formateString(str,obj){
		...
		return str.replace(/\{#(\w+)#\}/g,function(match,key){
			...
			if(key=="timelimitdate"){
				if(newobj["activedate"]==""||newobj["timelength"]==""){
					newobj[key]="未激活!"
				}else{
					newobj[key]=format(parseInt(newobj["activedate"])+parseInt(newobj["timelength"]), 'yyyy-MM-dd HH:mm:ss');
				}
			}
			if(key=="activedate"){
