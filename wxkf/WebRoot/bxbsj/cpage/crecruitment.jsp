...
">
	...
	<body ontouchstart >
		<div class="weui_tab" id="recruitment">
					...
					<div class="box nospacebox nopadding">
						<a href="javascript:;" class="weui_media_box weui_media_appmsg">
							...
							<div class="weui_media_bd">
								...
								<p v-cloak class="weui_media_desc margin-b-5">{{data.companyinfo.companyinfo_name}}<i v-if="data.companyinfo.companyinfo_status==1" class="iconfont icon-renzheng color-warning margin-l-5"></i><span class="float_r margin-r-15">{{data.companyinfo.companyinfo_pjob}}</span></p>
								<p class="weui_media_desc">在招职位<span class="color-success margin-r-15"> {{data.companyinfo.openid|JobNums}} </span>职业档案处理率<span class="color-warning"> {{data.companyinfo.openid|JobdelNums}}%</span> </p>
							</div>
			...
			<div class="weui_tabbar">
				...
				<a href="javascript:;" class="weui_tabbar_item margin-l-15 margin-r-15 bg-error">
					<p class="weui_tabbar_label" v-on:click="jobPost()"><i class="iconfont icon-toudi"></i>关闭职位</p>
				</a>
				-->
				<a href="javascript:;" class="weui_tabbar_item margin-l-15 margin-r-15 bg-success" >
					<p class="weui_tabbar_label" v-on:click="jobdel()"><i class="iconfont icon-ganxingqu"></i>关闭职位</p>
				</a>
			</div>
		</div>
					...
					var jobVue  =new Vue({
						...
						methods:{
							...
							jobdel:function(){
								...
								$.showLoading();
								$.ajax({
									url:"/wxkf/api/Job_del",
									data:{
										"job.job_id":<%=request.getParameter("job_id")%>,
									},
									success:function(data){
										if(data.result){
											$.toptip("职位已关闭!", 'success');
										}else{
