...
public class weixinServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			try {
				unionid=WeixinUtil.getUnionID(fromUserName);
			} catch (Exception e) {
				e.printStackTrace();
			}
			if(MessageUtil.MESSAGE_TEXT.equals(msgType)){
				if("1".equals(content)){
					...
					message = MessageUtil.initText(toUserName, fromUserName,MessageUtil.sixMenu());
				}else if("7".equals(content)){
					message = MessageUtil.initText(toUserName, fromUserName,MessageUtil.sevenMenu());
				}else if("8".equals(content)){
					message = MessageUtil.initText(toUserName, fromUserName,MessageUtil.eightMenu());
				}else if("9".equals(content)){
					message = MessageUtil.initText(toUserName, fromUserName,MessageUtil.nineMenu());
...
//					System.out.println(unionid.getNickname()+"关注了这个公共号!");
